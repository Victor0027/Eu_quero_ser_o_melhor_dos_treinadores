package ex1;
	...
	class Ash extends Treinador
	{
		...
		{
			if(PokeVivos==1&&Vet[0].HP<50.0)
			{
				return 5;//Foge
			}
			else if(Vet[0].HP<100&&Vet[0].HP>0.0&&Vet[0].UsouCura==0)
			{
				return 6;//Usa Cura
			}
	...
	class action
	{
		...
		public boolean AshAção(int j,Ash a,Trash b)
		{
			boolean acabou;
			int x=a.ação(j);
		...
		public boolean TrashAção(int j,Ash a,Trash b)
		{
			boolean acabou;
			int x=b.ação(j);
			Batalha bat=new Batalha(a,b);
		...
		public String vencedor(Ash a,Treinador b)
		{
			if(a.fugiu==true)
			{
	...
	public class Batalha_Pokemon
	{
		public static void main(String[] args)
		{
			...
			System.out.println(Blue.NomeTreinador+": Vá "+Blue.Vet[0].NomePoke+"!!");
			while(acabou==false)
			{
				if(Turno%2==0)
				{
